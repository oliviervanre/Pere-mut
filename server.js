...
app.post('/login', function(req, res){
	...
	if(!isValid(pattern_validator.login, req.body)) {
		...
		.select('status').select('email').select('match').select('ready');
		  			...
		  			permutant.save(function(error){
		  				if(error){
							...
							var mailOptions = {
							    ...
							    '<a href="http://localhost:8000/validInscription/'+dataToSave.tokken+'">Je click ici pour valider mon inscription !</a>' // plaintext body   
							};
...
var permutantlvl3 = [];
	...
	**/
	...
	.catch(function(err){
		...
		console.log(error, 'erreurfinal');
		console.log(combinaison, 'combinaison');
		res.send(user);
	})	*/
});
	/*firstRound().then( function(error, users) {
		...
		if ( error )
			...
			if(users.length > 0){
				for(user in users){
					if(users[user].destination.choix1 == client.residence || users[user].destination.choix2 == client.residence || users[user].destination.choix3 == client.residence){
						combinaison.twoParty.push({
							login : users[user].name,
							residence : users[user].residence,
							destination : users[user].destination,
							mail: users[user].email,
	...
	var query = Permutant.find();
  	query.where({grade: client.grade});
	query.or([{ residence : client.destination.choix1 }, { residence : client.destination.choix2 }, { residence : client.destination.choix3 }]);
	query.exec(function(error, users){
		if(error){
			console.error(error);
		}else{
			if(users.length > 0){
				...
				if(residencePossibility.length > 0){
					var query2 = Permutant.find();
					query2.where({grade: client.grade});
					query2.or(residencePossibility);
					residencePossibility = [];
					query2.exec(function(error, users2){
						if(error){
							console.error(error);
						}else{
							if(users2.length > 0){
								console.log(permutantlvl2, 'permutantlvl2');
								for(user2 in users2){
									if(users2[user2].destination.choix1 == client.residence || users2[user2].destination.choix2 == client.residence || users2[user2].destination.choix3 == client.residence){
										for(perm in permutantlvl2){
											if(permutantlvl2[perm].destination.choix1 == users2[user2].residence || permutantlvl2[perm].destination.choix2 == users2[user2].residence || permutantlvl2[perm].destination.choix3 == users2[user2].residence){
												combinaison.threeParty.push({
													position : '2',
													login : permutantlvl2[perm].name,
													residence : permutantlvl2[perm].residence,
													destination : permutantlvl2[perm].destination,
										...
										combinaison.threeParty.push({
											position : '3',
											login : users2[user2].name,
											residence : users2[user2].residence,
											destination : users2[user2].destination,
											mail: users2[user2].email
										});
									}else{
										residencePossibility.push({ residence : users2[user2].destination.choix1 });
										residencePossibility.push({ residence : users2[user2].destination.choix2 });
										residencePossibility.push({ residence : users2[user2].destination.choix3 });
										permutantlvl3.push(users2[user2]);
									}
								}
								if(residencePossibility.length > 0){
									var query3 = Permutant.find();
									query3.where({grade: client.grade});
									query3.or(residencePossibility);
									console.log(residencePossibility, 'residencepossibility3');
									residencePossibility = [];
									query3.exec(function(error, users3){
										if(error){
											console.error(error);
										}else{
											console.log(users3, 'USER3');
											if(users3.length > 0){
												console.log(permutantlvl3, 'permutantlvl3');
												for(user3 in users3){
													if(users3[user3].destination.choix1 == client.residence || users3[user3].destination.choix3 == client.residence || users3[user3].destination.choix3 == client.residence){
														for(perm in permutantlvl3){
															if(permutantlvl3[perm].destination.choix1 == users3[user3].residence || permutantlvl3[perm].destination.choix2 == users3[user3].residence || permutantlvl3[perm].destination.choix3 == users3[user3].residence){
																for(pperm in permutantlvl2){
																	if(permutantlvl2[pperm].destination.choix1 == permutantlvl3[perm].residence || permutantlvl2[pperm].destination.choix2 == permutantlvl3[perm].residence || permutantlvl2[pperm].destination.choix3 == permutantlvl3[perm].residence){
																		combinaison.fourParty.push({
																			position : '2',
																			login : permutantlvl2[pperm].name,
																			residence : permutantlvl2[pperm].residence,
																			destination : permutantlvl2[pperm].destination,
																...
																combinaison.fourParty.push({
																	position : '4',
																	login : users3[user3].name,
																	residence : users3[user3].residence,
																	destination : users3[user3].destination,
																	mail: users3[user3].email
																});
															}
														}
													}
